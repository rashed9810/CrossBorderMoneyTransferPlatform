...
interface ModalProps {
    isWithdrawIntoWalletModalOpen: boolean;
    setWithdrawIntoWalletModalOpen: (value: boolean) => void;
}

const WithdrawIntoWalletModal: React.FC<ModalProps> = ({ setWithdrawIntoWalletModalOpen, isWithdrawIntoWalletModalOpen }) => {
    const router = useRouter();

    const handleCloseModal = () => setWithdrawIntoWalletModalOpen(false);

    const handleWithdraw = (e: { preventDefault: () => void; }) => {
        e.preventDefault();
        router.push('/withdraw-successful')
    }

    return (
        <div className='w-full'>
            <Modal
                isOpen={isWithdrawIntoWalletModalOpen}
            ...
            >
                ...
                <div>
                    ...
                    <form onSubmit={handleWithdraw} className='space-y-3'>
                        ...
                        <div className="w-full relative">
                            <label className="block mb-1 ">You Will Get</label>
                            <div className='flex flex-row'>
                                <input
                                    type="number"
                                    name="converting"
                        ...
                        <div className='w-2/5 mx-auto py-3'>
                            <input className='w-full bg-[#3eae50] text-white px-2 py-1 rounded text-[10px] cursor-pointer' type="submit" value="Withdraw" />
                        </div>
                    </form>
                </div>
...
export default WithdrawIntoWalletModal;